...
namespace Sanicball
{
    ...
    public class MusicPlayer : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                aSource.volume = Mathf.Max(aSource.volume - 0.03f, 0.05f);
            }
            else
            {
                aSource.volume = Mathf.Min(aSource.volume + 0.03f, 0.5f);
            }

            //If it's not playing but supposed to play, change song
            if ((!aSource.isPlaying || GameInput.IsChangingSong()) && isPlaying && Application.isFocused)
            {
        ...
        private void ShuffleSongs()
        {
            ...
            for (var i = playlist.Length; i > 1; i--)
            {
