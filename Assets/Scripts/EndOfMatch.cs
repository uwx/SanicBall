...
namespace Sanicball
{
    public class EndOfMatch : MonoBehaviour
    {
        ...
        private HashSet<RacePlayer> movedPlayers = new HashSet<RacePlayer>();
        ...
        private void ActivateInner(RaceManager manager)
        {
            ...
            for (int i = 0; i < movablePlayers.Length; i++)
            {
                ...
                {
                    var rb = playerToMove.Ball.GetComponent<Rigidbody>();

                    rb.Move(spawnpoint, transform.rotation);
                    rb.linearVelocity = Random.insideUnitSphere * 0.5f;
                    rb.angularVelocity = new Vector3(0, Random.Range(-50f, 50f));
