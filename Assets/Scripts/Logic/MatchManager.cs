...
namespace Sanicball.Logic
{
    ...
    public class MatchManager : MonoBehaviour
    {
        ...
        /// </summary>
        public bool OnlineMode => messenger is OnlineMatchMessenger;

        /// <summary>
        /// Contains all clients connected to the game. In offline matches this will always only contain one client.
        /// </summary>
        public ReadOnlyCollection<MatchClient> Clients => clients.AsReadOnly();

        /// <summary>
        /// Contains all players in the game, even ones from other clients in online races
        /// </summary>
        public ReadOnlyCollection<MatchPlayer> Players => players.AsReadOnly();

        /// <summary>
        /// Current settings for this match. On remote clients, this is only used for showing settings on the UI.
        /// </summary>
        public MatchSettings CurrentSettings => currentSettings;

        public Guid LocalClientGuid => myGuid;

        public bool AutoStartTimerOn => autoStartTimerOn;
        public float AutoStartTimer => autoStartTimer;
        ...
        private void ClientLeftCallback(ClientLeftMessage msg, float travelTime)
        {
            ...
            var playersToRemove = players.Where(a => a.ClientGuid == msg.ClientGuid).ToList();
        ...
        public void InitOnlineMatch(WebSocket client, MatchState matchState)
        {
            ...
            {
                var p = new MatchPlayer(playerInfo.ClientGuid, playerInfo.CtrlType, playerInfo.CharacterId);
            ...
            //Create messenger
            var messenger = new OnlineMatchMessenger(client);
        ...
        {
            var myClient = clients.FirstOrDefault(a => a.Guid == myGuid);
        ...
        {
            var player = players.FirstOrDefault(a => a.ClientGuid == e.Movement.ClientGuid && a.CtrlType == e.Movement.CtrlType);
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.JoystickButton7))
            {
                ...
                {
                    var menu = Instantiate(pauseMenuPrefab);
        ...
        {
            IEnumerator Enumerator() { yield return SceneManager.LoadSceneAsync(lobbySceneName); }
        ...
        private void GoToStage()
        {
            ...
            IEnumerator Enumerator() { yield return SceneManager.LoadSceneAsync(targetStage.sceneName); }
            StartCoroutine(Enumerator());
        }
        ...
        {
            Time.timeScale = 1;
            CameraFade.StartAlphaFade(Color.black, false, 1f, 0f, () => StartCoroutine(QuitMatchInternal(reason)), true);
        }
        ...
        private void SpawnLobbyBall(MatchPlayer player)
        {
            ...
            var name = clients.First(a => a.Guid == player.ClientGuid).Name + " (" + GameInput.GetControlTypeName(player.CtrlType) + ")";
